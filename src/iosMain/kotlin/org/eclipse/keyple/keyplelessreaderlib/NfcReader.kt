...
internal object ReaderInstance {
    ...
    var getErrorMsg: (e:Exception) -> String = { e ->
        "Error: ${e.message}"
    }
        ...
        if (session == null) {
            ...
            MainScope().launch(Dispatchers.Main) {
                val newSession = NFCTagReaderSession(
                    NFCPollingISO14443, readerCallback,
    ...
    suspend fun sendCommand(isoApdu: NFCISO7816APDU): ByteArray = suspendCoroutine { cont ->
        ...
        val onCommandResult = { data: NSData?, sw1: uint8_t, sw2: uint8_t, error: NSError? ->
            if (error != null) {
                ...
                val dataArray = if (data != null && length > MIN_RESPONSE_LENGTH) {
                    data.toByteArray()
                } else ByteArray(0)

                val array = ByteArray(length) { pos ->
                    when (pos) {
                        length - 2 -> sw1.toByte()
                        length - 1 -> sw2.toByte()
                        else -> dataArray[pos]
...
internal class NativeNfcReaderCallback : NSObject(), NFCTagReaderSessionDelegateProtocol {
    ...
    override fun tagReaderSession(session: NFCTagReaderSession, didDetectTags: List<*>) {
        ...
        val onConnected = { error: NSError? ->
            error?.let {
                Napier.d(
                    tag = TAG,
                    message = "Error connecting session: ${error.localizedDescription}"
                )
    ...
    override fun tagReaderSession(session: NFCTagReaderSession, didInvalidateWithError: NSError) {
        Napier.d(
            tag = TAG,
            message = "Tag reader session invalidated: ${didInvalidateWithError.code} ${didInvalidateWithError.localizedDescription} userInfo:${didInvalidateWithError.userInfo}"
        )
